...
export async function getRecommendedQuestions({
  ...
  const allTags = interactedQuestions.flatMap((q) => q.tags.map((t) => t.toString()));

  const uniqueTagsIds = [...new Set(allTags)];

  const recommendedQuery: mongoose.QueryFilter<IQuestionDoc> = {
    _id: { $nin: interactedQuestionIds },
    author: { $ne: new Types.ObjectId(userId) },
    tags: { $in: uniqueTagsIds.map((id) => new Types.ObjectId(id)) },
  };
